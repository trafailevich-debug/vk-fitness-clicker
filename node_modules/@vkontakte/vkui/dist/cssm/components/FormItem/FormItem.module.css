...
:global(.vkuiInternalFormLayoutGroup--mode-horizontal) .densityCompact.withTop,
.densityCompact.withTop .removable {
  margin-block-start: calc(
    -1 *
    var(--vkui--spacing_size_2xs) -
...
@media (pointer: fine) and (min-width: 768px),(max-height: 414.9px) {
  /* stylelint-disable-next-line selector-pseudo-class-disallowed-list */
  :global(.vkuiInternalFormLayoutGroup--mode-horizontal) .densityNone.withTop,
  .densityNone.withTop .removable {
    margin-block-start: calc(
      -1 *
      var(--vkui--spacing_size_2xs) -
      var(--vkui--font_subhead--line_height--compact) -
