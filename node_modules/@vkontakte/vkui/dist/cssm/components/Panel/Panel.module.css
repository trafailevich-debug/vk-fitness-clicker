...
.viewWidthSmallTabletMinus .centered :global(.vkuiInternalPanelHeader--sep) + * {
  margin-block-start: calc(var(--vkui_internal--panel_header_height) + 5px);
}

@media (max-width: 767.9px) {
  /* stylelint-disable-next-line selector-pseudo-class-disallowed-list */
  .viewWidthNone .centered :global(.vkuiInternalPanelHeader--sep) + * {
    margin-block-start: calc(var(--vkui_internal--panel_header_height) + 5px);
  }
}
...
.viewWidthSmallTabletPlus .centered :global(.vkuiInternalPanelHeader--sep) + * {
